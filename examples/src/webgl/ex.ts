...
function createParagraph(elem, text) {
  const par = document.createElement(elem)
  par.innerHTML = text
...
function profile(gpu) {
  const par = createParagraph('p', 'Running reduction on ' + (gpu ? 'GPU' : 'CPU'))

  let i = 0
  const SAMPLES = 1000
  let total = 0
  const gpuReduce = gpuReduceCreate(data, 'max(a,b)')
  ...
  function loop() {
    ...
    const cmd = gpu ? () => gpuReduce() : () => cpuReduce(data, (a, b) => Math.max(a, b))
